...
%
% [I,tk1,tk2] = phasorPlot(G,S,sz,ex,fov,nor,txt,sty,cma)
...
if(size(nor,1)>1)
    INOR=nor;nor=3;
    if((size(INOR,1)~=size(S0,1))||(size(INOR,2)~=size(S0,2))),error('Intensity image shoudl be the same size as phasor coordinate image.');end   
end
if(nor==1)
